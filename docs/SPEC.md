...
- **`attributes.folder` non è affidabile per ogni torrent multi-file**: osservato un caso reale (season pack di "Stranger Things") dove l'istanza non lo popolava affatto, anche se il torrent aveva davvero una cartella. `Unit3dTrackerAdapter._normalize_pack_structure` gestisce solo il caso in cui `files[].name` riporta già il path relativo (es. `"Release.Name/Show.S01E02.mkv"`, non solo il nome nudo): quella è la cartella vera, estratta e tolta da `file_list`/`file_sizes` (mai lasciata annidata dentro il nome, il resto del motore assume sempre folder e filename separati). **Mai** un fallback su `attributes.name`: è un titolo "leggibile" per la UI (spazi), non necessariamente il vero nome di release scritto nel `.torrent` (punti) — un tentativo precedente lo usava come fallback ed è stato esplicitamente rimosso dopo aver causato in produzione un path sbagliato (spazi al posto dei punti) e quindi un recheck fallito (risalita al 99% e poi fallimento: il client cerca i file nel path che calcola dal `.torrent` reale, mai in quello che gli passiamo noi). Se `folder` manca e non c'è un path annidato, resta `None` qui — l'unica fonte davvero affidabile a quel punto è il `.torrent` stesso, letto al momento dell'esecuzione (vedi sotto).

### Fallback finale: leggere la cartella direttamente dal `.torrent`

Quando `candidate.folder` resta `None` per un pack (più file video) al momento dell'esecuzione, `app/executor.py::_ensure_folder_known` scarica il `.torrent` da `candidate.download_link` e ne decodifica il bencode (BEP3, parser minimale in `app/torrent_file.py` — nessuna dipendenza esterna) per leggere `info.name`, l'unico campo che il client torrent usa davvero per calcolare `save_path/info.name/...`. Il risultato viene persistito su `candidate.folder` (mai ri-scaricato ai retry successivi). Un fallimento (rete, formato) lascia `folder` invariato — le validazioni esplicite già esistenti (`_execute_season_pack`) segnalano poi l'assenza di una cartella, mai un nome indovinato. Limitato ai pack (>1 file video): per un file singolo un `folder` nullo è quasi sempre corretto così com'è, un fetch di rete per ogni esecuzione rallenterebbe inutilmente il caso comune.
